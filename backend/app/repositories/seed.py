...
def _seed_demo_accounts(session: Session, settings: Settings) -> bool:
    """Add only missing synthetic login identities."""
    changed = False
    for user, _password in get_demo_accounts(settings):
        if session.get(UserModel, user.id) is not None:
            continue
        session.add(
            UserModel(
                id=user.id,
...
def _seed_client_demo_case(session: Session, settings: Settings) -> None:
    """Create Elena's browser-visible demo case in server persistence."""
    case = CaseModel(
        id=DEMO_CASE_ID,
        owner_user_id=settings.demo_client_id,
    ...
    # Elena records no assets, deliberately, and this is the only section she
    # is missing. Keep this aligned with the browser seed.
    session.add(
        CaseTaskModel(
...
def reset_demo_data(settings: Settings) -> None:
    ...
    with session_factory() as session:
        _wipe_all(session)
        _seed_demo_accounts(session, settings)
        _seed_client_demo_case(session, settings)
...
def _seed_attorney_review_case(session: Session, settings: Settings) -> None:
    ...
    """
    if session.get(UserModel, ATTORNEY_REVIEW_CLIENT_ID) is None:
        session.add(
            UserModel(
...
def seed_demo_data_if_absent(settings: Settings) -> bool:
    """Reconcile missing synthetic fixtures without changing existing rows.
    ...
    This is the non-destructive counterpart: it identifies each synthetic user
    and case by its governed id, inserts only the missing fixtures, and leaves
    every existing row untouched.

    It exists for one deployment shape — a serverless target whose storage is
    per-instance and ephemeral. Usually a cold start is completely empty, but
    a warm or partially initialized instance can contain an unrelated row while
    still lacking a browser-visible demo case. A database-wide emptiness check
    stranded the attorney workspace in that state because ownership correctly
    refuses to let an attorney create the missing case through analyze.

    Returns True when at least one fixture was added.
    """
    init_db(settings)
    session_factory = get_sessionmaker()
    with session_factory() as session:
        changed = _seed_demo_accounts(session, settings)
        if session.get(CaseModel, DEMO_CASE_ID) is None:
            _seed_client_demo_case(session, settings)
            changed = True
        if session.get(CaseModel, ATTORNEY_REVIEW_CASE_ID) is None:
            _seed_attorney_review_case(session, settings)
            changed = True
        if session.get(CaseModel, INCOMPLETE_CASE_ID) is None:
            _seed_incomplete_case(session)
            changed = True
        if changed:
            session.commit()
...
def _seed_incomplete_case(session: Session) -> None:
    ...
    the second kind.
    """
    if session.get(UserModel, INCOMPLETE_CLIENT_ID) is None:
        session.add(
            UserModel(
