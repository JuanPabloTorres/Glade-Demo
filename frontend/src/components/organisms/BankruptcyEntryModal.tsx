    ...
    if (kind === "income") {
      ...
      onSave({
        ...
        value: {
          ...
          delinquentAmount: Number(delinquentAmount || 0),
          collateral: collateral || undefined,
          collectionLawsuit: flag,
          evidenceIds: [],
        },
      });
    } else if (kind === "asset") {
      onSave({
        ...
        value: {
          ...
          loanBalance: Number(monthlyPayment || 0),
          jointlyOwned: flag,
          evidenceIds: [],
        },
      });
  ...
  return (
    <Modal show={open} onClose={onClose} dismissible size="2xl">
      ...
      <form onSubmit={submit}>
        <ModalBody className="max-h-[72vh] space-y-5 overflow-y-auto">
          {kind === "income" ? (
            <>
              <div>
                ...
                <Select id="income-category" value={primary} onChange={(event) => setPrimary(event.target.value)} required>
                  <option value="">Seleccione</option>
                  {INCOME_CATEGORIES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                </Select>
              </div>
              <div>
                <Label htmlFor="income-source">Fuente o patrono</Label>
                <TextInput id="income-source" value={secondary} onChange={(event) => setSecondary(event.target.value)} required />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div><Label htmlFor="gross-amount">Ingreso bruto</Label><TextInput id="gross-amount" type="number" min="0" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required /></div>
                <div><Label htmlFor="net-amount">Ingreso neto</Label><TextInput id="net-amount" type="number" min="0" step="0.01" value={netAmount} onChange={(event) => setNetAmount(event.target.value)} /></div>
              </div>
          ...
          {kind === "debt" ? (
            <>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div><Label htmlFor="debt-balance">Balance</Label><TextInput id="debt-balance" type="number" min="0" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required /></div>
                <div><Label htmlFor="debt-payment">Pago mensual</Label><TextInput id="debt-payment" type="number" min="0" step="0.01" value={monthlyPayment} onChange={(event) => setMonthlyPayment(event.target.value)} /></div>
                <div><Label htmlFor="debt-delinquent">Atraso</Label><TextInput id="debt-delinquent" type="number" min="0" step="0.01" value={delinquentAmount} onChange={(event) => setDelinquentAmount(event.target.value)} /></div>
              </div>
          ...
          {kind === "evidence" ? (
            <>
              ...
              <div><Label htmlFor="evidence-file">Archivo</Label><FileInput id="evidence-file" onChange={(event) => setSecondary(event.target.files?.[0]?.name ?? "")} helperText="El demo conserva únicamente el nombre y estado del archivo en este navegador." /></div>
        ...
        <ModalFooter>
          <Button type="submit" className="glade-button">Guardar</Button>
          <Button type="button" color="alternative" onClick={onClose}>Cancelar</Button>
        </ModalFooter>
      </form>
    </Modal>
  );
